...
@with_registry
def sim_print_plan_capture(oregistry):
    yield from bps.null()
    sim_det = oregistry["sim_det"]
    sim_motor = oregistry["sim_motor"]
    det_read = sim_det.read()
    print(f"sim_print_plan_return(): {sim_motor.position=}, {det_read=}")
    results.append(det_read)
    return results  # Python generator "return" value
